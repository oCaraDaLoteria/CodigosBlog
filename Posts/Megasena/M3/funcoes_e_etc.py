...
class Loterias:
    ...
    # (Output) item : um vetor com 'qtd_numeros' numeros diferentes dentro do intervalo especificado
        ...
        if self.nome_loteria=='Quina':
            ...
            qtd_numeros = 5
        item = random.sample(intervalo,  qtd_numeros)
...
def M3_Histograma_Linhas(dados, nome_figura='HistSimples.png'):
    fig, ax = plt.subplots()
    def autolabel(rects):
        # attach some text labels
        for rect in rects:
            ...
            ax.text(rect.get_x()+rect.get_width()/2., 1.005*height, '%.1f'%float(height),
                    ha='center', va='bottom',zorder=500)
    bins = range(0,7)
    histograma_dados, bins = np.histogram(dados, bins=bins,normed=True)
    width = np.diff(bins)
    center = (bins[:-1] + bins[1:]) / 2
    rectshd = ax.bar(center, histograma_dados*100, align='center', width=width,alpha = 0.7,edgecolor='k')
    autolabel(rectshd) # coloca valores sobre as barras
    ax.set_xticks(center) # defino a posicao de onde colocar o numero de linhas
    ax.set_xticklabels([1,2,3,4,5,6]) # mostra o numero de linhas
    ax.set_ylim([0,100])
    ax.set_xlabel('Num. da Linha')
    ax.set_ylabel('Probabilidade (%)')
